...
def extract():
    ...
    for partition in unchecked_partition:
        ...
        file_store_path = os.path.join(
            os.environ.get("DATA_DIR"), f"{gz_name}")
            ...
            for line in file_content_lines:
                ...
                if ('Computer Science' in fos):
                    ...
                    if jso['venue'] != '' and jso['abstract'] != '' and jso['year'] != None:
                        jso['title'] = zlib.compress(
                            str.encode(jso['title'], encoding='utf-8'), 9)
                        jso['abstract'] = zlib.compress(
                            str.encode(jso['abstract'], encoding='utf-8'), 9)
                        jso['authors'] = zlib.compress(
                            str.encode(jso['authors'], encoding='utf-8'), 9)
                        cs_data.append(jso)
...
def export(start, amount):
    ...
    output_s2_incomplete_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_{int(start) + 1}_to_{int(start) + int(amount)}#incomplete.data")
    if os.path.exists(output_s2_incomplete_data_file_name):
        os.remove(output_s2_incomplete_data_file_name)

    output_s2_complete_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_{int(start) + 1}_to_{int(start) + int(amount)}.data")
    while amount_left > 0:
        data_out = []
    ...
    os.rename(output_s2_incomplete_data_file_name,
              output_s2_complete_data_file_name)
    print(output_s2_complete_data_file_name +
          f" saved with {amount} completed data")
...
def export_rand():
    ...
    output_s2_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_rand.{rand_sequence}.data")
    with open(output_s2_data_file_name, "w") as leaned_raw_topic_data:
        leaned_raw_topic_data.write("\n".join(data_out))
        print(output_s2_data_file_name +
              f" saved with {len(data_out)} completed data")
...
def export_with_id(ids_file):
    ...
    output_s2_incomplete_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_with_id.{tail}#incomplete.data")
    if os.path.exists(output_s2_incomplete_data_file_name):
        os.remove(output_s2_incomplete_data_file_name)
    output_s2_complete_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_with_id.{tail}.data")

    start_idx = 0
    ids_left = len(ids)
    while ids_left > 0:
        end_idx = start_idx + \
            data_loading_line_limit if ids_left > data_loading_line_limit else start_idx + ids_left
    ...
    os.rename(output_s2_incomplete_data_file_name,
              output_s2_complete_data_file_name)
    print(output_s2_complete_data_file_name +
          f" saved with {len(ids)} completed data")
...
def search_matching_result_with_title_in_s2_by_partition(partition_name_list_and_matched_jso_data):
    ...
    print(
        f'finished searching from: {partition_name_list[0]} to {partition_name_list[len(partition_name_list) - 1]} in {len(titles_by_partition)} s2 data')
...
def matching_arxiv_data_by_topic(cate, src):
    ...
    with Pool(p_number) as pool:
        rs = pool.map_async(
            search_matching_result_with_title_in_s2_by_partition, process_map_arg)
    ...
    output_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 'index', "arxiv_" + cate['name'] + "_data_in_s2id.txt")
    with open(output_data_file_name, "w") as leaned_raw_data:
        leaned_raw_data.write("\n".join(data_out))
        print(output_data_file_name + " saved")

    output_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 'index', "arxiv_" + cate['name'] + "_data_in_s2id_title_compare.txt")
    with open(output_data_file_name, "w") as leaned_raw_data:
        leaned_raw_data.write("\n".join(data_v_out))
        print(output_data_file_name + " saved")
...
def extract_from_arxiv_cate_id_list(cate_name, amount=None):
    src = os.path.join(os.environ.get("DATA_DIR"), 'index',
                       "arxiv_" + cate_name + "_data_in_s2id.txt")
    id_list = []
    with open(src) as f:
        lines = f.readlines()
        for id in lines:
    ...
    output_s2_data_file_name = os.path.join(os.environ.get(
        "DATA_DIR"), 's2_sample', f"completed_s2_arxiv_{cate_name}_{sufix}_.data")
    with open(output_s2_data_file_name, "w") as leaned_raw_topic_data:
        leaned_raw_topic_data.write("\n".join(data_out))
        print(output_s2_data_file_name +
              f" saved with {len(data_out)} completed data")
...
def count_arxiv_data_by_cate():
    ...
    for c in cates:
        if c != 'cs':
            cate_count += 1
            src = os.path.join(os.environ.get("DATA_DIR"),
                               'index',  "arxiv_" + c + "_data_in_s2id.txt")
            with open(src) as f:
                lines = f.readlines()
